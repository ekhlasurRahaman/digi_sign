...
if (isset($_POST['submit'])){
	...
	$required_fields = array("vein_data");
	validate_presences($required_fields);
	$fields_with_max_lengths = array("vein_data" => 100);
	validate_max_lengths($fields_with_max_lengths);
	
	if(empty($errors)){
	$vein_data = $_POST["vein_data"];	
	$found_public = attempt_login_public($vein_data);//username and password varification and return true or false
	if ($found_public) {
		// Success
		//mark user as login
		...
		$message = "You are not Registered.To Register Click Sign Up.";
		
	}
}elseif(!empty($errors)){
	$errors = $errors['vein_data'];
}	
}else {
          ...
          <ul class="list-unstyled">
            ...
            <li style="color:red;font-size: 20px;">
              <i class= "glyphicon glyphicon-hand-right"> </i>
			   <?php if(!empty($message)){ echo $message;}elseif(!empty($errors)){ echo $errors;} ?>
